...
# Copy all the constants from your mpc.py file
...
# Rest of your code, but with fixed get_video_chunk call
def main():
    ...
    while True:  # serve video forever
        # FIXED: Handle different number of return values
        result = net_env.get_video_chunk(bit_rate)
        
        if len(result) == 7:  # Without sleep_time
            delay, buffer_size, rebuf, video_chunk_size, next_video_chunk_sizes, end_of_video, video_chunk_remain = result
            sleep_time = 0
        else:  # With sleep_time (8 values)
            delay, sleep_time, buffer_size, rebuf, video_chunk_size, next_video_chunk_sizes, end_of_video, video_chunk_remain = result
        ...
        for full_combo in CHUNK_COMBO_OPTIONS:
            ...
            # calculate total rebuffer time for this combination
