...
# IMPROVED TRAINING PARAMETERS (30-45 minutes target)
S_INFO = 6
S_LEN = 8
A_DIM = 6
ACTOR_LR_RATE = 0.0001    # ORIGINAL learning rate (slower)
CRITIC_LR_RATE = 0.001    # ORIGINAL learning rate (slower)
NUM_AGENTS = 16           # ORIGINAL number of agents
TRAIN_SEQ_LEN = 100       # ORIGINAL sequence length
MODEL_SAVE_INTERVAL = 100 # ORIGINAL save interval
EPOCH = 1000              # MUCH longer training to get 30-45 minutes
...
RANDOM_SEED = 42
RAND_RANGE = 1000
SUMMARY_DIR = './improved_quick_results'  # CHANGED to separate results
LOG_FILE = './improved_quick_results/log'
TEST_LOG_FOLDER = './improved_quick_test_results/'
TRAIN_TRACES = './improved_quick_traces/'
MONITOR_FILE = './improved_quick_results/monitor_log.txt'
NN_MODEL = None
...
def testing(epoch, nn_model, log_file):
    # Quick testing on every 5th epoch to monitor progress
    if epoch % 5 == 0:
        with tf.Session() as sess:
            log_file.write('\n')
            log_file.write('Testing on network ' + str(epoch) + '\n')
            
            # Just run a quick test to monitor progress
            log_file.write('avg reward: -10.0 (placeholder)\n')  # You can add real testing here
            log_file.flush()
...
def run_real_testing(epoch, nn_model, log_file):
    ...
    if epoch % 100 == 0:  # Only run every 100 epochs
        ...
        # Save the current model for testing
...
def central_agent(net_params_queues, exp_queues):
    ...
    with tf.Session() as sess, open(LOG_FILE + '_test', 'w') as test_log_file:
        ...
        while epoch < EPOCH:
            ...
            # Run quick test every 100 epochs (like original)
            if epoch % 100 == 0:
                run_real_testing(epoch, saver.save(sess, SUMMARY_DIR + "/temp_model.ckpt"), test_log_file)

            # Save model every 100 epochs (like original)
...
def main():
    log_monitor("=== Starting Long Training Session (30-45 minutes target) ===")
    log_monitor("Parameters: NUM_AGENTS={0}, EPOCH={1}, TRAIN_SEQ_LEN={2}".format(NUM_AGENTS, EPOCH, TRAIN_SEQ_LEN))
    log_monitor("Using ORIGINAL learning rates and parameters from multi_agent.py")
    log_monitor("Estimated training time: {0:.1f} minutes".format(EPOCH * 2 / 60.0))  # Rough estimate with longer epochs
    ...
    # Create expanded trace set if it doesn't exist
    if not os.path.exists(TRAIN_TRACES) or len(os.listdir(TRAIN_TRACES)) < 20:
        log_monitor("Creating expanded training trace set...")
        import shutil
        source_traces = './cooked_traces/'  # Use main training traces
        
        # Create improved_quick_traces directory if it doesn't exist
        if not os.path.exists(TRAIN_TRACES):
            os.makedirs(TRAIN_TRACES)
            
        # Copy 20 traces for improved training
        trace_files = ['trace_{0}.txt'.format(i*5) for i in range(20)]  # Every 5th trace
